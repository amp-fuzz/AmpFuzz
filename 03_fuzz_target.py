...
def fuzz(executable, ports, args, dryrun=False, inetd=False):
    ...
    if dryrun:
        ...
        print(' '.join([ampfuzz_bin + '/fuzzer', '-c', targets, '-i', in_dir, '-o', out_dir, '-t', track,
                        '--target_addr', '127.0.0.1:' + port, '--', fast] + args))
    else:
        exec_command(ampfuzz_bin + '/fuzzer', ['-c', targets, '-i', in_dir, '-o', out_dir, '-t', track,
                                               '--target_addr', '127.0.0.1:' + port, '--', fast] + args)
    # build/fuzzer -c ./targets.json -i in -o out -t ./avahi-daemon.track --target_addr 127.0.0.1:5353 -- ./avahi-daemon.fast
...
def main():
    ...
    # fuzz?
    fuzz(executable, ports, fuzzee_args, dryrun=args.no_fuzz, inetd=args.inetd)
