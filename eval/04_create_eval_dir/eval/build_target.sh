...
BASE_IID_FILE=$(realpath "$1")
TARGET=$2
TARGET_IID_FILE=$(realpath "$3")

TARGET_CID_FILE=${TARGET_IID_FILE%%.iid}.cid
...
docker commit -c 'CMD ["/bin/bash"]' $(cat ${TARGET_CID_FILE}) > ${TARGET_IID_FILE}
