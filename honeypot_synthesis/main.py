...
def generate_honeypot_function(lines):
    ...
    for i, (constraints, output) in enumerate(p.parse()):
        ...
        print(to_python(total_path, simplify=True, name=f'check{i}'))
        print()

        output_exprs = [parse_expr(f"(+ {output_byte.strip()})") for output_byte in output]
        print(to_python_bytes(output_exprs, simplify=True, name=f'output{i}'))
        print()
        check_out.append((f'check{i}', f'output{i}'))
    print('def gen_reply(pkt_in):')
    for check, output in check_out:
        print(f'    if {check}(pkt_in):')
        print(f'        return {output}(pkt_in)')
...
def main():
    ...
    generate_honeypot_function(lines)
