...
lazy_static! {
    ...
    static ref EARLY_TERMINATION: EarlyTermination = match env::var(EARLY_TERMINATION_VAR){
        Ok(v) => {
            match v.as_str() {
                "none" => {debug!("early termination disabled"); EarlyTermination::None},
                "dynamic" => {debug!("dynamic early termination enabled"); EarlyTermination::Dynamic},
                "static" => {debug!("static early termination enabled"); EarlyTermination::Static},
                _ => {debug!("full early termination enabled"); EarlyTermination::Full}
                }
            }
...
pub unsafe extern "C" fn epoll_wait(epfd: c_int, events: *mut epoll_event, maxevents: c_int, timeout: c_int) -> c_int {
    //TODO: only trigger if fuzz_fd is waited on
    __angora_check_terminate();
    __angora_listen_ready();
